...
rm -f reasoning_context.md task_decomposition.md Qwen3.8-Max_file_index.md Qwen3.8-Max.md Accio-Desktop.md

# 2. Build a unified payload map containing instructions and source code definitions
SYSTEM_PROMPT="You are the BabyShark Master Rich-Context Agentic Framework. Analyze the context and write three distinct sections using explicit structural Markdown headers: '### REASONING CONTEXT', '### TASK DECOMPOSITION', and '### FILE INDEX'."
...
echo "$RAW_RESPONSE" | sed -n '/### FILE INDEX/,$p' > Qwen3.8-Max_file_index.md

# Create structural mirrors for the orchestrator expectations
echo "$RAW_RESPONSE" > Qwen3.8-Max.md
echo "$RAW_RESPONSE" > Accio-Desktop.md

# 5. Validation Check
if [ ! -s reasoning_context.md ] || [ ! -s task_decomposition.md ] || [ ! -s Qwen3.8-Max_file_index.md ]; then
