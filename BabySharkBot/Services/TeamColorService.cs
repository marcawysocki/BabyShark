...
namespace BabySharkBot.Services
{
    ...
    public static class TeamColorService
    {
        ...
        public static WorkerAssignment GetAssignmentForNewWorker(int currentWorkerCount)
        {
            ...
            if (workerIndex >= 15)
            {
                // Worker 16 and beyond: Speed mining phase roles
                // For now, let's assign them to the remaining 4th slots
                if (workerIndex == 15) return new WorkerAssignment { prefix = "T", workerNum = 4, color = Teal, pairIndex = 0 }; // T4 (16th)
                
                // Generic fallback for extra workers
